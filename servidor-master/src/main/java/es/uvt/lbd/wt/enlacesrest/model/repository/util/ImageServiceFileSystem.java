...
public class ImageServiceFileSystem implements ImageService {
  ...
  @Override
  public String saveImage(MultipartFile file, Long id) throws AppException {
    if (file.isEmpty()) {
      throw new AppException("No se ha enviado ningún fichero");
    }
    ...
    try (InputStream inputStream = file.getInputStream()) {
      Files.copy(
          inputStream,
          getRootLoc().resolve(id + getExtension(filename)),
          StandardCopyOption.REPLACE_EXISTING);
    } catch (IOException e) {
      e.printStackTrace();
      throw new AppException("Problema procesando el fichero");
    }
  ...
  @Override
  public ImageDTO getImage(String imagePath, Long id) throws AppException {
    try {
      InputStream is =
          new FileInputStream(properties.getImagesPath() + id + getExtension(imagePath));
      ...
      e.printStackTrace();
      throw new AppException("Problem while getting the image");
    }
  }

  private Path getRootLoc() {
    if (rootLoc == null) this.rootLoc = Paths.get(properties.getImagesPath());
    return rootLoc;
  }
